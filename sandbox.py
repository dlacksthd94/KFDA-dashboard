...
class MiamiPlot():
    def __init__(self) -> None:
        ...
        # filtering
        LIST_DRUG = ["statin_bf"]
        LIST_BIO_MARKER = ["Cholesterol", "Free Cholesterol", "Triglycerides"]

        df_drug = df_outcome[df_outcome["drug_exposure"].isin(LIST_DRUG)]  # select drug
        ...
        df_lipo = df_meta[df_meta["title"].isin(dict_title)]  # select metabolite
        df_lipo = df_lipo.drop(columns=["units", "Group", "Subgroup"])
        df_lipo["title"].unique()

        # preprocess
        dict_size = {
            "Chylomicrons and Extremely Large": "XXL",
            "Very Large": "XL",
            "Large": "L",
            "Medium": "M",
            "Small": "S",
        ...
        def preprocess(row):
            title = row["title"]
            row = row.append(
                pd.Series(dict_title[title], index=["size", "lipoprotein", "bio_marker"])
            )
        ...
        df_model = df_merge[
            ...
            + list(filter(lambda val: val.endswith(str(MODEL_NUM)), df_merge.columns))
        ]

        # calculate CI
        df_model[f"CI{MODEL_NUM}"] = (
            CI_LEVEL * df_model[f"se{MODEL_NUM}"]
        )  # / np.sqrt(df_model[f'n{MODEL_NUM}'])
        ...
        # fig = go.Figure()
        fig = make_subplots(
            rows=1, cols=len(LIST_BIO_MARKER), shared_yaxes=True, horizontal_spacing=0
        )
        range_max = df_model[f"beta{MODEL_NUM}"].max() + df_model[f"CI{MODEL_NUM}"].max()
        range_min = df_model[f"beta{MODEL_NUM}"].min() - df_model[f"CI{MODEL_NUM}"].max()
        ...
        for i, (bio_marker, df_group) in enumerate(df_model.groupby("bio_marker")):
            _ = fig.update_xaxes(title_text=bio_marker, side="top", row=1, col=i + 1)
            
        _ = fig.update_layout(
            autosize=True,
            yaxis=dict(
                titlefont_size=16, tickfont_size=12, title="coefficient", showspikes=True
            ),
            xaxis=dict(titlefont_size=16, tickfont_size=12),
